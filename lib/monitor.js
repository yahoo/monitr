...
var reqCounter = null, //Singleton instance to monitor requests, connections and data transferred
    healthStatus = null; //Singleton instance to monitor status of app
...
function HealthStatus() {
    this._majorStatus = -1; //0 can be a valid status so using -1
    this._minorStatus = -1;
    this._timestamp = 0;
}

HealthStatus.prototype.setHealthStatus = function(majorStatus, minorStatus) {
    var tmp = parseInt(majorStatus, 10);
    this._majorStatus = !isNaN(tmp) ? tmp : -1;

    tmp = parseInt(minorStatus, 10);
    this._minorStatus = !isNaN(tmp) ? tmp : -1;
...
function setupHealthStatus() {
    ...
    process.monitor.getMajorStatus = function() {
        return healthStatus._majorStatus;
    };

    process.monitor.getMinorStatus = function() {
        return healthStatus._minorStatus;
    };
...
module.exports.enableHealthInformation = function() {
    setupHealthStatus();
};
